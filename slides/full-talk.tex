...
% keywords
alsoletter={_, .},
morekeywords={set.seed},
keywordstyle={\color{Keywords}\bfseries},
deletekeywords={ts, mat, time, sample, response, stat, col, by},
% additional keywords
morekeywords={[2]@response, group, stratum, stat, response, reps, 
alternative, alternatives, combine, {keep_dist}, seed, size, pvalues, keep_dist},
...
\begin{itemize}
\itemsep15pt
\item \citet{fisher_design_1935} introduced permutation tests for the analysis of randomized experiments
\item They rely on assumptions about randomization or exchangeabilityto generate the distribution of a test statistic under the null hypothesis
\item They were largely replaced by parametric hypothesis tests because they were computationally infeasible, but are making a comeback
\end{itemize}
\vspace{10pt}
...
\frame{
\frametitle{Teaching Statistics with Permutation Tests}
\textbf{Goals for \texttt{permuter}:}\\

\begin{itemize}
\itemsep 15pt
\item Create a comprehensive package for permutation tests\\
\begin{itemize}
\itemsep 5pt
\item Building blocks of permutation tests
\item Transparent, not a black box, to facilitate understanding
\end{itemize}
\item Write code with algorithms and computation in mind
\item Compile examples and case studies alongside the package
\end{itemize}
}
...
test_distr <- matrix(0, nrow = 1e5, ncol = length(categories))
pvals <- rep(0, length(categories))
colnames(test_distr) <- names(pvals) <- categories

# Loop over rating categories
for(col in categories){
  set.seed(678)
  observed <- tst_statistic(macnell[ ,col])
  distr <- stratified_two_sample(response = macnell[ ,col], 
                                 group = macnell$taidgender, 
                                 stratum = macnell$tagender,
                                 stat = "mean", reps = 1e5)
  test_distr[ ,col] <- distr
  pvals[col] <- t2p(observed, distr, alternative = "two-sided")
}

# NPC 
omnibus_pvalue <- npc(pvals, test_distr, combine = "fisher", 
...
\vspace{10pt}
\textbf{Slides:}
\url{https://github.com/kellieotto/useR2016}
