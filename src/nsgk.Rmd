...
This vignette works through the data cleaning and inter-rater reliability analysis for the data from Naomi Stark and Gilbert Kliman (henceforth called NSGK).

For details, see https://github.com/statlab/nsgk.
...
```{r summary_stats}
video_means <- t(sapply(nsgk_mat, function(tt){
                        sapply(tt, mean)
                        }))
head(video_means)

video_sds <- t(sapply(nsgk_mat, function(tt){
                        sapply(tt, sd)
                        }))
head(video_sds)
...
npc_results <- lapply(nsgk_mat, function(tt){
